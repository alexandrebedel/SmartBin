      ...
      <div>
        ...
        />
          ...
          <p className="mt-2 text-sm text-gray-600">
            Date sélectionner : <span className="font-medium text-gray-800">{formatDateToFrench(selectedDate)}</span>
          </p>
...
export default function StatsDetail({ params }: { params: { id: string } }) {
  ...
  const result = (statsDetails?.trashData?.filter((trash: any) => trash.createdAt.includes(selectedDate)).reduce((acc, cur) => {
